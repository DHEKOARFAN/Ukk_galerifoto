...
<head>
    ...
    <style>
        .bg-image {
            background-image: url('../assets/img/lucu.jpeg');
            height: 100vh;
        }
    </style>
</head>
...
<body class="bg-image">
    ...
    <select class="form-control" name="albumid" required>
        ...
        while ($data_album = mysqli_fetch_array($sql_album)) { ?>
            <option value="<?php echo htmlspecialchars($data_album['albumid']) ?>">
    ...
    <button type="submit" class="btn btn-primary mt-2" name="tambah">Tambah Data</button>
                                                        ...
                                                        <div class="modal-body">
                                                            <form action="../config/aksi_foto.php" method="POST" enctype="multipart/form-data">
                                                                ...
                                                                <select class="form-control" name="albumid">
                                                                    ...
                                                                    $sql_album = mysqli_query($koneksi, "SELECT * FROM album WHERE userid='$userid'");
                                                                    while ($data_album = mysqli_fetch_array($sql_album)) { ?>
                                                                        <option <?php if ($data_album['albumid'] == $data['albumid']) { ?> selected="selected" <?php } ?> value="<?php echo htmlspecialchars($data_album['albumid']) ?>">
                                                                            <?php echo htmlspecialchars($data_album['namaalbum']) ?>
                                                                        </option>
                                                                    <?php } ?>
                                                                </select>
                                                        ...
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body">
                                                                <form action="../config/aksi_foto.php" method="POST">
                                                                    <input type="hidden" name="fotoid" value="<?php echo $data['fotoid'] ?>">
                                                                    Apakah anda yakin akan menghapus data <strong> <?php echo htmlspecialchars($data['judulfoto']) ?> </strong> ?
                                                            </div>
                                                            <div class="modal-footer">
            ...
            <footer class="d-flex justify-content-center border-top mt-3 bg-light fixed-bottom">
                <p>&copy;2024 Projek Galeri Foto | DHEKO MEI ARFANDI</p>
            </footer>
